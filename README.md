...
## Current Status
- Phase 3 baseline in progress: FastAPI backend, Pydantic contracts, encrypted local profile vault, deterministic opportunity qualification, local Tailor drafts, approval-gated action proposals, audit log, configurable external execution endpoint, live-safe crypto RSS ingestion, JSON-backed persistent store, and Tauri-ready Svelte command deck are operational. X, Discord, marketplace, and board-specific crypto sources are disabled until real live adapters/credentials are configured; no fake/demo signal generation is used. Semantic/vector memory, platform-specific executors, and scoped autopilot sends remain pending.
...
- `POST /api/opportunities/{opportunity_id}/draft`
- `POST /api/opportunities/{opportunity_id}/actions/propose`
- `GET /api/actions/proposals`
- `POST /api/actions/{action_id}/approve` / `POST /api/actions/{action_id}/reject`
- `POST /api/actions/{action_id}/execute` for approved actions with a configured real executor adapter
...
- Qualification and Tailor drafts are currently deterministic/local. External sends/submissions require an approved action proposal and a configured real executor adapter; the default executor returns 503 instead of pretending to send.

## Quick Start

See `docs/RUN_FULL_PROJECT.md` for the full backend + desktop + feedback-loop runbook.
